...
> **심화 각주 — 그런데 Windows/macOS에서는?** 컨테이너가 공유하는 커널은 정확히는 **리눅스 커널**이다. 그래서 Windows·macOS에서는 Docker Desktop이 내부에 가벼운 리눅스 VM(Windows는 WSL2)을 하나 띄우고, 모든 컨테이너는 그 VM 안에서 돈다. "그럼 결국 VM 아닌가?" 싶겠지만, VM은 딱 한 대만 두고 그 위의 컨테이너 수십 개가 커널을 공유하는 구조라서 컨테이너의 가벼움은 그대로 유지된다. 스터디에서 이 질문이 나오면 이렇게 답하면 된다.
...
# macOS/Linux: ./gradlew bootBuildImage --imageName=spring-petclinic:1.0
```
...
- buildpacks 이미지는 층이 훨씬 많고 정교하다. JVM 메모리 자동 설정(5장 심화 참고), 비-루트 사용자 실행 같은 전문가의 레시피가 들어 있다. **세 줄짜리도 잘 돌지만, 그 차이를 눈으로 보는 게 이 심화의 목적이다.**
...
> **다른 방법(선택)**: 이미지 안에 셸이 있다면 `docker exec petclinic-app kill 1`로 컨테이너 안의 자바 프로세스를 직접 죽여 크래시를 흉내 낼 수도 있다. 다만 최근 buildpacks 기본 이미지에는 셸이 없는 경우가 있으니, 스터디 전에 본인 이미지로 되는지 먼저 확인하자.
...
여기서 `{"status":"UP"}`이 나와야 **앱까지** 준비된 것이다. "컨테이너 Up ≠ 앱 UP"을 이 화면으로 눈에 익히면 된다.
...
2) 이미지 빌드    .\gradlew.bat bootBuildImage --imageName=spring-petclinic:1.0
